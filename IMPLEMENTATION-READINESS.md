...
- **Blocks:** the **non-negotiable conformance gate** that gates *every* phase (18.6, 20 "Every phase is gated by the cross-client conformance vectors"). Foundational to Phase 0.1 (P0.1-WEB-2) and all crypto.
...
- **M-3 — Visual/product design system.** `OPEN-DECISIONS.md` backlog "Design": "the visual & product design direction is undefined … nothing pins the look-and-feel yet." Web 15 gives structure, responsive breakpoints, dark-first, shadcn components — enough to build *functionally* with shadcn defaults, but brand/design tokens are unspecified. Affects Phase 4.5 polish; not a functional blocker.
...
- **M-5 — Enterprise Keycloak realm specifics.** `client_id`/`authority` are runtime (per-account `config.json`), so this is fine; only the realm/claims mapping details would come from a deployment, not the spec. Minor.
