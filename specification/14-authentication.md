...
- **Server-side password pepper (client unchanged).** As a server-side defense-in-depth (ratified 2026-07-07, [OPEN-DECISIONS](https://github.com/Nyxite/Nyxite/blob/main/docs/OPEN-DECISIONS.md)), the account-password verifier is now **Argon2id over `HMAC-SHA256(password, pepper)`**, where the **pepper is a server secret** held outside the DB ([server 08](https://github.com/Nyxite/NyxiteServer)). This is **entirely server-side**: the **web client is unchanged** — it still sends the raw password over TLS at login and never holds or applies the pepper. The pepper is account-auth only; it does **not** touch content-key derivation or the recovery-phrase Argon2id path ([07 §7.6](07-key-and-device-management.md)).
...
- **Password + TOTP:** the app renders a native login form and `POST`s the credentials (password + the current TOTP code) to the Nyxite server's auth endpoint via `fetch`. The server verifies the Argon2id password verifier — computed server-side over an **HMAC-SHA256 pepper pre-hash** ([§14.1](#141-identity-provider)) — and the TOTP code, and returns **access + refresh tokens**. The static-export shell sends the raw password only transiently over TLS, applies **no pepper of its own**, and retains no copy.
